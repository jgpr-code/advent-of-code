...
struct TaskData {
    ...
    best_release: HashMap<(String, i128), i128>, // (node, time) -> released
}
impl TaskData {
    ...
    fn do_prune(&self, node: String, remaining_time: i128, released: i128) -> bool {
        // let key = (node, remaining_time);
    ...
    fn update_best(&mut self, node: String, remaining: i128, released: i128) {
        let key = (node, remaining);
        if self.best_release.contains_key(&key) {
    ...
    fn bfs_with_prune(&mut self, start: String) {
        let mut queue: VecDeque<(String, i128, i128, HashSet<String>)> = VecDeque::new(); // at_min, released, remaining
        queue.push_back((start, 30, 0, HashSet::new()));
        while let Some((node, remaining, released, opened)) = queue.pop_front() {
            if remaining <= 0 {
                continue;
            } // no time left => nothing to do
              // try open
            let flow = self.nodes[&node];
            if !opened.contains(&node) && flow > 0 {
                let new_remaining = remaining - 1; // open costs 1
                let new_released = released + open(flow, remaining);
                let mut new_opened = opened.clone();
                new_opened.insert(node.clone());
                if !self.do_prune(node.clone(), new_remaining, new_released) {
                    self.update_best(node.clone(), new_remaining, new_released);
                    queue.push_back((node.clone(), new_remaining, new_released, new_opened));
                }
            }
            // insert neighbors
            let neighs: Vec<String> = self.adjacency[&node].clone();
            for neigh in neighs {
                let new_remaining = remaining - 1; // moving costs 1
                let new_released = released;
                let new_opened = opened.clone();

                if !self.do_prune(neigh.clone(), new_remaining, new_released) {
                    self.update_best(neigh.clone(), new_remaining, new_released);
                    queue.push_back((neigh.clone(), new_remaining, new_released, new_opened));
                }
            }
        }
    }
...
fn parse_input(input: &str) -> Result<TaskData> {
    ...
    for line in input.lines() {
        ...
        best_release.insert((name.clone(), 30), 0);
    }
    Ok(TaskData {
        nodes,
        adjacency,
