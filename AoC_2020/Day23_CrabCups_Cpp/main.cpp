...
struct FastCupSimulator {
  ...
  list<ll>::const_iterator next(list<ll>::const_iterator it) const {
    if (it ==
        myCups
            .cend()) {  // end should never be reached unless the list is empty
      return it;
    }
    ++it;
    if (it == myCups.cend()) {
  ...
  list<ll>::const_iterator find_destination_cup(ll current_cup,
                                                const list<ll> picked_up_cups) {
...
int main() {
  list<ll> inputCups = {3, 1, 8, 9, 4, 6, 5, 7, 2};
